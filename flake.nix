    ...
    let
      perSystem = (system:
        let
          ...
          pkgsMuslAarch64 = import nixpkgs {
            inherit system overlays;
            crossSystem = {
          ...
          enclaverCrate = pkgsMuslAarch64.rustPlatform.buildRustPackage {
            pname = "enclaver-app";
            version = "0.1.0";
            src = ./.;
          ...
          # A specific, version-pinned kernel is essential for determinism.
          # This uses kernel 6.12 with the required settings for Nitro Enclaves.
            ...
            let
              ...
              enclaveRootFs = pkgs.runCommand "enclave-rootfs" {
                nativeBuildInputs = [
                  ...
                  pkgsMuslAarch64.stdenv.cc.libc
                  entrypointScript
                  pkgs.patchelf
                ];
              } ''
                mkdir -p $out/bin $out/lib $out/etc/enclaver
                cp -L ${pkgsMuslAarch64.stdenv.cc.libc}/lib/* $out/lib/
                ...
                for binary in $out/bin/*; do
                  if [ -f "$binary" ] && [ -x "$binary" ]; then
                    patchelf --set-interpreter /lib/ld-musl-aarch64.so.1 "$binary" || true
                  fi
                done
              ...
              enclaverEif = nitro-lib.buildEif {
                name = "enclaver-deterministic-eif";
                kernel = "${kernel}/Image";
        ...
        noApp = pkgsMuslAarch64.writeShellApplication {
          name = "entrypoint";
          runtimeInputs = [];
          text = ''
        ...
        in
        {
          lib.makeAppEif = makeAppEif;

          packages = {
            default = defaultBuild.eif;
            enclaver-run = pkgsMuslAarch64.rustPlatform.buildRustPackage {
              pname = "enclaver-run";
              version = "0.1.0";
              src = ./.;
